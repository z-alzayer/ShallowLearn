    ...
    def _load_image(self):
        ...
        with rio.open(self.path) as src:
            ...
            for i in range(src.count):
                band_desc = src.descriptions[i] if src.descriptions[i] else f"Band_{i+1}"
...
class SatelliteImageCollection(ABC):
    ...
    def get_common_bands_array(self) -> np.ndarray:
        ...
        band_indices = sorted(
            [first_image.band_order[b] for b in common_bands]
        )
...
class Sentinel2ImageCollection(SatelliteImageCollection):
    ...
    def _get_sorted_image_files(self) -> List[Path]:
        ...
        files = list(self.directory.glob("*cropped.vrt"))
        return sorted(files, key=extract_date)
...
def create_satellite_collection(directory: str, satellite_type: Optional[str] = None) -> SatelliteImageCollection:
    """
