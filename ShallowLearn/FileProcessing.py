...
def find_matching_files(list1: List[str], list2: List[str]) -> List[MatchingFiles]:
    """Matches Sentinel-2 L1C and L2A files with identical acquisition dates."""

    def extract_date(file: str, prefix: str) -> datetime:
    ...
    l1c_dates = [(extract_date(f, "MSIL1C_"), f) for f in list1]
    l2a_dates = [(extract_date(f, "MSIL2A_"), f) for f in list2]

    # Filter invalid filenames and create date-keyed dictionaries
    valid_l1c = {d.date(): (d, f) for d, f in l1c_dates if d}
    valid_l2a = {d.date(): (d, f) for d, f in l2a_dates if d}

    # Find common dates
    common_dates = valid_l1c.keys() & valid_l2a.keys()

    return [
        MatchingFiles(
            valid_l1c[date][0],  # datetime object
            valid_l1c[date][1],  # L1C path
            valid_l2a[date][1],  # L2A path
        )
...
def check_in_string(input_string, extension=None, other_string=None):
    ...
    return extension_present and (
        other_string_present if other_string is not None else True
    )
...
def check_values_in_filenames(file_list, values):
    ...
    return [
        ...
        if any(value in file_name for value in values)
    ]
...
def order_band_names_noreg(file_names, band_mapping=band_mapping):
    ...
    return sorted(
        file_names, key=lambda x: band_mapping[get_band_identifier(x)]["index"]
    )
...
def order_by_band(files, order=band_mapping.keys()):
    ...
    bands_files = {
        re.search(r"(B[\dA]+)_(\d+)m", file).group(1): file
        for file in highest_resolution_files
    }
