...
class QuickLookVisualizer:
    ...
    def __init__(self, quicklook_filter):
        """Initialize with a QuickLookFilter instance from ml module"""
        self.filter = quicklook_filter
        self.class_dict = quicklook_filter.class_dict
        self.transformed_data = quicklook_filter.transformed_data
        self.labels = quicklook_filter.labels
        self.thumbnails = quicklook_filter.thumbnails
        self.products = quicklook_filter.products
    
    def plot_clusters_scatter(self, figsize=(10, 8), save_path=None):
        """Create scatter plot of clusters in reduced dimensional space"""
        if self.transformed_data is None:
            raise ValueError("No transformed data available. Run process_products first.")
        ...
        # Set labels
        method_name = self.filter.reducer.get_name()
    ...
    def plot_thumbnails_on_scatter(self, show_points=True, show_thumbnails=True, 
                                  zoom=0.1, figsize=(12, 10), max_images=50, 
                                  sample_method='random', add_borders=True,
                                  save_path=None):
        """
        ...
        if show_thumbnails and self.thumbnails:
            ...
            if sample_method == 'all' or max_images is None:
                ...
                for label in unique_labels:
                    ...
                    n_samples = min(max_images // len(unique_labels), len(label_indices))
                    if n_samples > 0:
                        sampled = np.random.choice(label_indices, n_samples, replace=False)
                        indices.extend(sampled)
            else:  # random
                n_images = min(len(self.thumbnails), max_images)
                indices = np.random.choice(len(self.thumbnails), n_images, replace=False)
        ...
        x_min, x_max = self.transformed_data[:, 0].min(), self.transformed_data[:, 0].max()
        y_min, y_max = self.transformed_data[:, 1].min(), self.transformed_data[:, 1].max()
        ...
        # Labels and legend
        method_name = self.filter.reducer.get_name() if hasattr(self.filter, 'reducer') else 'Reduced'
        ax.set_xlabel(f'{method_name} Component 1', fontsize=12)
        ax.set_ylabel(f'{method_name} Component 2', fontsize=12)
    ...
    def plot_temporal_distribution(self, figsize=(14, 8), save_path=None):
        ...
        df = pd.DataFrame({
            ...
            'label': self.labels[:len(dates)]  # In case some dates failed to parse
        })
        ...
        colors_dict = {label: self.class_dict.get(label, ('#808080', f'Cluster_{label}'))[0] 
                      for label in unique_labels}
        
        monthly_counts.plot(kind='bar', stacked=True, ax=ax2, 
    ...
    def plot_cluster_statistics(self, figsize=(12, 8), save_path=None):
        ...
        for label in np.unique(self.labels):
            mask = self.labels == label
            cluster_products = [self.products[i] for i in range(len(self.products)) if mask[i]]
        ...
        colors = [self.class_dict.get(label, ('#808080', ''))[0] 
                 for label in np.unique(self.labels)]
        ...
        brightness_means = [stats['mean_brightness'] for stats in cluster_stats.values()]
        ...
        for sat in all_satellites:
            satellite_data[sat] = [stats['satellites'].get(sat, 0) 
                                 for stats in cluster_stats.values()]
        
        bottom = np.zeros(len(names))
        for i, sat in enumerate(all_satellites):
            axes[1, 1].bar(names, satellite_data[sat], bottom=bottom, 
                          label=sat, alpha=0.8)
    ...
    def create_cloudcover_meshgrid(self, ax, resolution=50, alpha=0.3):
        ...
        try:
            ...
            # Create contour plot
            contour = ax.contourf(xx, yy, grid_values, levels=20, alpha=alpha, cmap='Blues_r')
    ...
    def plot_publication_quality(self, method_name="PCA", show_thumbnails=True, 
                                show_meshgrid=True, thumbnail_sample='cluster',
                                figsize=(7, 4), dpi=300, save_path=None):  # Figure size for A4 page
        """
        ...
        fig, ax = plt.subplots(figsize=figsize, dpi=100)  # matplotlib will scale for save
        ...
        if show_thumbnails and self.thumbnails:
            ...
            for i in indices:
                try:
                    ...
                    resized_img = resize(
                        thumbnail,
                        output_shape=(int(thumbnail.shape[0] * zoom), int(thumbnail.shape[1] * zoom)),
                        anti_aliasing=True,
                        preserve_range=True,
    ...
    def generate_all_plots(self, output_dir="publication_plots", dpi=300):
        ...
        # Get method name
        method_name = self.filter.reducer.get_name() if hasattr(self.filter, 'reducer') else 'Unknown'
        satellite_name = list(set(p.satellite for p in self.products))[0] if self.products else 'unknown'
        
        # 1. Basic scatter plot
        fig1, ax1 = self.plot_clusters_scatter(save_path=f"{output_dir}/{satellite_name}_{method_name}_scatter.png")
