...
class SatelliteImageProcessor:
    ...
    def detect_satellite_type(self, image: SatelliteImage) -> str:
        ...
        if hasattr(image, 'tags'):
            ...
            if any(level in processing_level for level in ['Level-1C', 'Level-2A', 'L1C', 'L2A']):
                return 'sentinel-2'
    ...
    def process_image(self, image: SatelliteImage) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if satellite_type == 'sentinel-2':
                ...
                band_order = {band: i for i, band in enumerate(sorted(image.present_bands))}
            ...
            if self.clip_percent > 0:
                processed_image = clip_image(processed_image, clip_percent=self.clip_percent)
...
class QuickLookProcessor:
    ...
    def process_images(self, 
                      images: List[Union[SatelliteImage, str]],
                      create_metadata: bool = True) -> 'QuickLookProcessor':
        """
        ...
        for img in images:
            try:
                ...
                if isinstance(img, (str, Path)):
                    ...
                    if any(pattern in str(img_path).lower() for pattern in ['s2a_', 's2b_', 'msil1c', 'msil2a', '.safe']):
    ...
    def reduce_dimensions(self) -> 'QuickLookProcessor':
        ...
        if not self.processed_images:
            raise ValueError("No processed images available. Call process_images first.")
        
        print(f"Applying {self.reducer.get_name()} to {len(self.processed_images)} images...")
        ...
        print(f"Reduced from {flattened.shape[1]} to {self.transformed_data.shape[1]} dimensions")
    ...
    def cluster_images(self) -> 'QuickLookProcessor':
        ...
        if self.transformed_data is None:
            raise ValueError("No transformed data available. Call reduce_dimensions first.")
        ...
        for img in self.images:
            ...
            if hasattr(img, 'tags') and img.tags:
                metadata.update({
                    ...
                    'aot_retrieval_method': img.tags.get('AOT_RETRIEVAL_METHOD', 'N/A'),
                })
            
            # Add MTL metadata for Landsat
            if hasattr(img, 'mtl_tags') and img.mtl_tags:
                metadata.update({
                    'landsat_scene_id': img.mtl_tags.get('LANDSAT_SCENE_ID', 'Unknown'),
    ...
    def run_complete_analysis(self, 
                            images: List[Union[SatelliteImage, str]]) -> 'QuickLookProcessor':
        """
...
def quick_analysis(image_paths: List[str],
                  method: str = 'pca',
                  clustering: str = 'dbscan',
                  target_bands: Optional[List[str]] = None) -> QuickLookProcessor:
    """
