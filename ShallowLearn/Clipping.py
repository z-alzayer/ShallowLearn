...
class BasicClipper:
    def __init__(self, shapefile, raster_path):
        """ Initialize the BasicClipper with a shapefile and a raster file path """
        self.shapefile = gpd.read_file(shapefile)
        self.raster = rio.open(raster_path)

    def clip_raster(self, shape_index=0):
        """ Clip the raster with the first geometry from the shapefile """
        # Extract the geometry in GeoDataFrame to use as a mask
        geometry = self.shapefile.geometry.iloc[shape_index]
        # Clip the raster using the geometry
        clipped_image, clipped_transform = mask(self.raster, [geometry], crop=True)
...
if __name__ == "__main__":
    ...
    raw_files = "/mnt/sda_mount/ukis_masks/"
    output_dir = "/mnt/sda_mount/ukis_masks_clipped"
    files = get_files_with_extension(raw_files, ".tiff")
    # files = [i for i in files if 'benthic_2' in i]

    # files = [i for i in files if 'udm2' not in i]
    for file in files:
        print(file)
        create_directory_if_not_exists(os.path.join(output_dir, file[:-5]))
        output_directory = os.path.join(output_dir, file[:-5])
