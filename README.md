...
That is the whole setup. The data is in the repository, so there is nothing to download or configure. `uv.lock` pins the exact package versions the committed outputs were produced with.
...
data/        the three arrays they use
```
...
10 m, all 13 MSI bands as delivered (DN = reflectance × 10 000).

`data/reef_clear.npy` — a ribbon reef on the outer Great Barrier Reef, from 2020-05-31, 656×333, same 13-band layout.
