...
class TestBCET:
    ...
    def test_bcet_extreme_values(self, sample_image_float):
        ...
        result = BCET(
            sample_image_float, min_value=-1000, max_value=5000, desired_mean=2000
        )
...
class TestScalingFunctions:
    ...
    def test_scaler_with_outliers(self):
        ...
        for band in range(3):
            ...
            assert (
                np.std(robust_no_outliers) <= np.std(std_no_outliers) * 2
...
class TestTransformIntegration:
    ...
    def test_transform_reversibility_approximation(self, sample_image_uint8):
        ...
        original_min, original_max = (
            np.min(sample_image_uint8),
            np.max(sample_image_uint8),
        )
        ...
        reverse_result = (lce_result / 255.0) * (
...
class TestTransformEdgeCases:
    ...
    def test_transforms_with_infinite_values(self):
        ...
        try:
            ...
            assert not np.any(
                np.isinf(result[1:, 1:, :])
            )  # Non-inf areas should be finite
        except (ValueError, RuntimeWarning):
